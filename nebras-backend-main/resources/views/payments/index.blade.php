            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3 d-flex justify-content-between align-items-center px-3">
                        <h6 class="text-white text-capitalize mb-0">{{ __('app.payments') }}</h6>
                ...
                <div class="card-body px-0 pb-2">
                    <div class="table-responsive px-3">
                        <table id="payments-table" class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.paid amount')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.discount')}}</th>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tbody id="installments-tbody">
                                    <tr>
                                        <td colspan="5" class="text-center">No installments available</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
...
<script>
    ...
    function handleTable(){
        $("#payments-table").dataTable().fnDestroy();
        $('#payments-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                "url": "{{ LaravelLocalization::localizeUrl(route('payments.index')) }}",
                "data": {},
                "type": "GET"
            },
            order: [[0, 'desc']],
            columns: [
                {
                    ...
                    render: function(data, type, row) {
                        if (type === 'display' && row.is_installment_payment) {
                            return '<span class="rounded-circle bg-success d-inline-block me-1" style="width:10px;height:10px;vertical-align:middle" title="{{ __("app.installment payment") }}"></span>' + data;
                        }
                        return data;
                    }
                },
    ...
    function showPaymentModal(paymentId) {
        $.ajax({
            url: `/payments/${paymentId}`, // adjust to your route
            type: "GET",
            success: function (payment) {
                ...
                if (payment.payment_items && payment.payment_items.length > 0) {
                    payment.payment_items.forEach(function(item) {
                        const paymentTypeLabel = item.payment_type === 'monthly' 
                    ...
                    paymentItemsHtml += `
                        <tr class="table-secondary">
                            <td colspan="4" class="text-end fw-bold">Total Before Discount:</td>
                            <td class="fw-bold">${payment.total_before_discount || "$0.00"}</td>
                        </tr>
                        <tr class="table-secondary">
                            <td colspan="4" class="text-end fw-bold">
                                Discount${payment.coupon_code ? ` (${payment.coupon_code} - ${payment.discount_percentage || 0}%)` : ''}:
                            </td>
                            <td class="fw-bold">${payment.discount_amount || "$0.00"}</td>
                        </tr>
                        <tr class="table-success">
                            <td colspan="4" class="text-end fw-bold">Total After Discount:</td>
                            <td class="fw-bold">${payment.total_after_discount || "$0.00"}</td>
                        </tr>
                    `;
                } else {
                    ...
                    payment.installments_by_course.forEach(function(courseGroup) {
                        ...
                        installmentsHtml += `
                            <tr class="table-primary">
                                <td colspan="5" class="fw-bold">${courseGroup.course_name || "-"}</td>
                            </tr>
                        `;
                        
                        // Installments for this course
                        if (courseGroup.installments && courseGroup.installments.length > 0) {
                            ...
                            installmentsHtml += '<tr><td colspan="5" class="text-center text-muted">No installments available for this course</td></tr>';
                        }
                    });
                } else {
