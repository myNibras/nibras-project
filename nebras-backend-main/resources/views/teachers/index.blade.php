                ...
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3">
                        <div class="d-flex align-items-center gap-2">
                            <h6 class="text-white text-capitalize mb-0">{{ __('app.teachers') }}</h6>
                        ...
                        <div class="d-flex gap-3">
                            <a href="{{ route('teachers.create') }}" class="btn btn-primary mb-0 px-3 py-2 d-flex align-items-center justify-content-center gap-2"><i class="fa-solid fa-plus"></i> <span style="height:16px;">{{ __('app.add new') }}</span></a>
                        ...
                        <table id="teachers-table" class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.position') }}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.years of experience') }}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.reviews') }}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.number of classes') }}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.number of students') }}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.updated at') }}</th>
...
<script>
    ...
    function handleTable(){
        ...
        $('#teachers-table').DataTable({
            ...
            columns: [
                { data: 'name' },
                { data: 'position' },
                { data: 'years_of_experience', className: 'text-center' },
                { data: 'reviews', orderable: true, searchable: false },
                { data: 'classes_count', orderable: true, searchable: false },
                { data: 'students_count', orderable: true, searchable: false },
                { data: 'updated_at', orderable: true, searchable: false },
                { data: 'status', orderable: false, searchable: false },
                { data: 'action', orderable: false, searchable: false },
            ],
            columnDefs: [
                { targets: 0, className: 'ps-4 py-2 pe-2' },
                { targets: 1, className: 'ps-4 py-2 pe-2' },
                { targets: 2, className: 'ps-4 py-2 pe-2 text-center' },
                { targets: 3, className: 'ps-4 py-2 pe-2 text-center' },
                { targets: 4, className: 'ps-4 py-2 pe-2 text-center' },
                { targets: 5, className: 'ps-4 py-2 pe-2 text-center' },
                { targets: 6, className: 'ps-4 py-2 pe-2 text-center' },
    ...
    function openAdditionalInfoModal() {
        $.ajax({
            url: "{{ route('teachers.additional-info.get', ['type' => 'teachers']) }}",
            type: 'GET',
    ...
    $('#additionalInfoForm').on('submit', function(e) {
        ...
        $('#additionalInfoErrors').addClass('d-none').html('');
        $.ajax({
            url: "{{ route('teachers.additional-info.store') }}",
            type: 'POST',
            data: $(this).serialize(),
            success: function(response) {
