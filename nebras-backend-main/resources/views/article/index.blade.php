@extends('layouts.app')
@section('title'){{ __('app.article') }}@endsection
            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3">
                        <div class="d-flex align-items-center gap-2">
                            <h6 class="text-white text-capitalize mb-0">{{ __('app.article') }}</h6>
                        ...
                        <div class="d-flex gap-3">
                            <a href="{{ route('article.create') }}" class="btn btn-primary mb-0 px-3 py-2 d-flex align-items-center justify-content-center gap-2"><i class="fa-solid fa-plus"></i> <span style="height:16px;">{{ __('app.add new') }}</span></a>
                ...
                <div class="card-body px-0 pb-2">
                    <div class="table-responsive px-3">
                        <table id="article-table" class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.short description')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.status')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.creation date')}}</th>
...
<script>
    ...
    function handleTable(){
        $("#article-table").dataTable().fnDestroy();
        $('#article-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                "url": "{{ LaravelLocalization::localizeUrl(route('article.index')) }}",
                "data": {},
                "type": "GET"
            },
            order: [[0, 'desc']],
    ...
    function openAdditionalInfoModal() {
        $.ajax({
            url: "{{ route('article.additional-info.get', ['type' => 'article']) }}",
            type: 'GET',
    ...
    $('#additionalInfoForm').on('submit', function(e) {
        ...
        $('#additionalInfoErrors').addClass('d-none').html('');
        $.ajax({
            url: "{{ route('article.additional-info.store') }}",
            type: 'POST',
            data: $(this).serialize(),
            success: function(response) {
