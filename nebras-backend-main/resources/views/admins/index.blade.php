            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3 d-flex justify-content-between align-items-center px-3">
                        <h6 class="text-white text-capitalize mb-0">{{ __('app.admins') }}</h6>
                        <div class="d-flex gap-3">
                            <a href="{{ route('admins.create') }}" class="btn btn-primary mb-0 px-3 py-2 d-flex align-items-center justify-content-center gap-2"><i class="fa-solid fa-plus"></i> <span style="height:16px;">{{ __('app.add new') }}</span></a>
                        ...
                        <table id="admins-table" class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.role')}}</th>
...
<script>
    ...
    function handleTable(){
        $("#admins-table").dataTable().fnDestroy();
        $('#admins-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                "url": "{{ LaravelLocalization::localizeUrl(route('admins.index')) }}",
                "data": {},
                "type": "GET"
            },
            order: [[0, 'desc']],
