            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3">
                        <h6 class="text-white text-capitalize px-3">{{ __('app.academic levels') }} - {{ __('app.add new') }}</h6>
                ...
                <div class="card-body px-0 pb-2">
                    ...
                    <form action="{{ route('academic-levels.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row mx-3 row-gap-3 w-100">
                                        ...
                                        @foreach (\App\Models\AcademicLevel::QUOTE_ICON_COLORS as $color)
                                            <div class="form-check form-check-inline m-0">
                                                <input class="form-check-input" type="radio" name="quote_icon_color" id="quote_icon_color_{{ $loop->index }}" value="{{ $color }}"
                                                    {{ old('quote_icon_color', \App\Models\AcademicLevel::DEFAULT_QUOTE_ICON_COLOR) === $color ? 'checked' : '' }}>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <input type="file" id="thumbnail_male" name="thumbnail_male" data-plugins="dropify" data-height="150" data-allowed-file-extensions="png jpg jpeg webp" />
                                    <small class="mt-2 d-block">Allow (jpg, jpeg, png, webp)</small>
                                </div>
                                <div class="col-md-6">
                                    <label for="thumbnail_female">{{ __('app.thumbnail_female') }} - {{ __('app.size')}} (318x318px)</label>
                                    <input type="file" id="thumbnail_female" name="thumbnail_female" data-plugins="dropify" data-height="150" data-allowed-file-extensions="png jpg jpeg webp" />
                                    <small class="mt-2 d-block">Allow (jpg, jpeg, png, webp)</small>
                                </div>
                            </div>
                        </div>
