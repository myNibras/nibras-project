@extends('layouts.app')
@section('title'){{ __('app.news') }} - {{ __('app.edit') }}@endsection
            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3">
                        <h6 class="text-white text-capitalize px-3">{{ __('app.news') }} - {{ __('app.edit') }}</h6>
                ...
                <div class="card-body px-0 pb-2">
                    ...
                    <form action="{{ route('news.update', $news->id) }}" method="POST" enctype="multipart/form-data" id="news-form">
                        ...
                        <div class="row mx-3 row-gap-3 w-100">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="title_ar" id="title_ar" placeholder="{{ __('app.title') }} (AR)"
                                        class="form-control" value="{{ old('title_ar', $news->title_ar) }}" maxlength="50" required>
                                    @error('title_ar')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="title_en" id="title_en" placeholder="{{ __('app.title') }} (EN)"
                                        class="form-control" value="{{ old('title_en', $news->title_en) }}" maxlength="50" required>
                                    @error('title_en')
                                        <span class="text-danger small">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="row mb-3">
                                {{-- Small Description AR --}}
                                <div class="col-md-6">
                                    <label for="small_description_ar" class="form-label fw-bold">{{ __('app.small description') }} (AR) <small class="text-muted">({{ __('app.max') }}: 150 {{ __('app.characters') }})</small></label>
                                    <textarea name="small_description_ar" id="small_description_ar" class="form-control" rows="3" maxlength="150" required>{{ old('small_description_ar', $news->small_description_ar) }}</textarea>
                                    <small class="text-muted"><span id="small_description_ar_count">0</span>/150</small>
                                ...
                                <div class="col-md-6">
                                    <label for="small_description_en" class="form-label fw-bold">{{ __('app.small description') }} (EN) <small class="text-muted">({{ __('app.max') }}: 150 {{ __('app.characters') }})</small></label>
                                    <textarea name="small_description_en" id="small_description_en" class="form-control" rows="3" maxlength="150" required>{{ old('small_description_en', $news->small_description_en) }}</textarea>
                                    <small class="text-muted"><span id="small_description_en_count">0</span>/150</small>
                            ...
                            <div class="row mb-3">
                                {{-- Expiry Date --}}
                                <div class="col-md-6">
                                    <label for="expiry_date" class="form-label fw-bold">{{ __('app.expiry date') }}</label>
                                    <input type="text" name="expiry_date" id="expiry_date" class="form-control" 
                                        placeholder="{{ __('app.expiry date') }}" 
                                        value="{{ old('expiry_date', $news->expiry_date ? $news->expiry_date->format('Y-m-d H:i') : '') }}">
                                    @error('expiry_date')
                                        <span class="text-danger small">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-check form-switch mt-4">
                                        <input class="form-check-input" 
                                            ...
                                            role="switch" 
                                            id="status"
                                            {{ old('status', $news->status ?? 1) ? 'checked' : '' }}>
...
<script>
    $(document).ready(function() {
        ...
        var contentAr = {!! json_encode(old('full_description_ar', $news->full_description_ar)) !!};
        var contentEn = {!! json_encode(old('full_description_en', $news->full_description_en)) !!};
        if (contentAr) {
        ...
        var expiryDatePicker = flatpickr("#expiry_date", {
            enableTime: true,
        ...
        updateCharCount('small_description_ar', 'small_description_ar_count', 150);
        updateCharCount('small_description_en', 'small_description_en_count', 150);
    });
</script>
@endpush
