            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3 d-flex justify-content-between align-items-center px-3">
                        <h6 class="text-white text-capitalize mb-0">{{ __('app.coupons') }}</h6>
                        <div class="d-flex gap-3">
                            <a href="{{ route('coupons.create') }}" class="btn btn-primary mb-0 px-3 py-2 d-flex align-items-center justify-content-center gap-2"><i class="fa-solid fa-plus"></i> <span style="height:16px;">{{ __('app.add new') }}</span></a>
                ...
                <div class="card-body px-0 pb-2">
                    <div class="table-responsive px-3">
                        <table id="coupons-table" class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    <th class="text-secondary font-weight-bolder opacity-7 text-center" style="width:100px">{{ __('app.id')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.coupon code')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.discount percentage')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.expiry date')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.type')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.owner student')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.used by')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.used/total')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.used date')}}</th>
...
<script>
    ...
    function handleTable(){
        $("#coupons-table").dataTable().fnDestroy();
        $('#coupons-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                "url": "{{ LaravelLocalization::localizeUrl(route('coupons.index')) }}",
                "data": {},
                "type": "GET"
            },
            order: [[0, 'desc']],
            columns: [
                ...
                { data: 'owner_student', orderable: false, searchable: false },
                { data: 'used_by', orderable: false, searchable: false },
                { data: 'used_total', orderable: false, searchable: false },
                { data: 'used_date', orderable: false, searchable: false },
            ...
            columnDefs: [
                ...
                { targets: 9, className: 'ps-4 py-2 pe-2' }
            ],
            language: {
                paginate: {
