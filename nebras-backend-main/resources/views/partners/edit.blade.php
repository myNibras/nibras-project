@extends('layouts.app')
@section('title'){{ __('app.partners') }} - {{ __('app.edit') }}@endsection
            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3">
                        <h6 class="text-white text-capitalize px-3">{{ __('app.partners') }} - {{ __('app.edit') }}</h6>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="name_ar" id="name_ar" placeholder="{{ __('app.name_ar') }}"
                                        class="form-control" value="{{ old('name_ar', $partner->name_ar) }}">
                                    @error('name_ar')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="name_en" id="name_en" placeholder="{{ __('app.name_en') }}"
                                        class="form-control" value="{{ old('name_en', $partner->name_en) }}">
                                    @error('name_en')
                                        <span class="text-danger small">{{ $message }}</span>
                                    @enderror
                                </div>
...
<script>
    $(document).ready(function() {
        // Handle logo removal via Dropify
        var drEvent = $('#logo').dropify();

        drEvent.on('dropify.afterClear', function(event, element){
            // Mark logo for removal when user clears it
            $('#remove_logo').val('1');
        });
