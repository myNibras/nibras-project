            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3 d-flex justify-content-between align-items-center px-3">
                        <div class="d-flex align-items-center gap-2">
                            <h6 class="text-white text-capitalize mb-0">{{ __('app.partners') }}</h6>
                        ...
                        <div class="d-flex gap-3">
                            <a href="{{ route('partners.create') }}" class="btn btn-primary mb-0 px-3 py-2 d-flex align-items-center justify-content-center gap-2"><i class="fa-solid fa-plus"></i> <span style="height:16px;">{{ __('app.add new') }}</span></a>
                ...
                <div class="card-body px-0 pb-2">
                    <div class="table-responsive px-3">
                        <table id="partners-table" class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    <th class="text-secondary font-weight-bolder opacity-7 text-center" style="width:100px">{{ __('app.id')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.logo')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.name_ar')}}</th>
                                    <th class="text-secondary font-weight-bolder opacity-7">{{ __('app.name_en')}}</th>
...
<script>
    ...
    function handleTable(){
        $("#partners-table").dataTable().fnDestroy();
        $('#partners-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                "url": "{{ LaravelLocalization::localizeUrl(route('partners.index')) }}",
                "data": {},
                "type": "GET"
            },
            order: [[0, 'desc']],
    ...
    // Additional Information Modal Functions
    function openAdditionalInfoModal() {
        // Load existing data if available
        $.ajax({
            url: "{{ route('partners.additional-info.get', ['type' => 'partners']) }}",
            type: 'GET',
            ...
            error: function(xhr) {
                // If no data found, just open empty modal
                $('#additionalInfoForm')[0].reset();
                $('#additionalInfoErrors').addClass('d-none').html('');
                var modal = new bootstrap.Modal(document.getElementById('additionalInfoModal'));
                modal.show();
    ...
    $('#additionalInfoForm').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('partners.additional-info.store') }}",
            type: 'POST',
            data: $(this).serialize(),
            success: function(response) {
