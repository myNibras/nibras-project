            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3">
                        <h6 class="text-white text-capitalize px-3">{{ __('app.roles') }} - {{ __('app.add new') }}</h6>
                                        ...
                                        <div class="bg-light p-3 border rounded mb-2">
                                            <div class="d-flex justify-content-between align-items-center mb-2">
                                                <h6 class="text-dark fw-bold text-capitalize mb-0">
                                                    <input type="checkbox" id="check_all_{{ str_replace(' ', '_', $type) }}" class="check-all-type" data-type="{{ str_replace(' ', '_', $type) }}">
                                                    <label for="check_all_{{ str_replace(' ', '_', $type) }}" class="text-dark fw-bold text-capitalize mb-0">{{ str_replace('_', ' ', $type) }}</label>
                                                </h6>
                                            <div>    
                                        </div>
                                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    // When "Select All" for a type is clicked
    ...
    // When an individual checkbox changes, update the "Select All" checkbox
    document.querySelectorAll('.perm-item').forEach(function (perm) {
        perm.addEventListener('change', function () {
            const classes = Array.from(this.classList);
            const typeClass = classes.find(c => c.startsWith('perm-type-'));
            const type = typeClass.replace('perm-type-', '');
