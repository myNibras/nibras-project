@extends('layouts.app')
@section('title'){{ __('app.testimonials') }} - {{ __('app.edit') }}@endsection
            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3 d-flex justify-content-between align-items-center px-3">
                        <h6 class="text-white text-capitalize px-3">{{ __('app.testimonials') }} - {{ __('app.edit') }}</h6>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="name" id="name" placeholder="{{ __('app.name') }}"
                                        class="form-control" value="{{ old('name', $testimonial->name) }}">
                                    @error('name')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="status" id="status" class="form-control form-select px-2">
                                        <option value="pending" {{ old('status', $testimonial->status) == 'pending' ? 'selected' : '' }}>{{ __('app.pending') }}</option>
                                        <option value="approved" {{ old('status', $testimonial->status) == 'approved' ? 'selected' : '' }}>{{ __('app.approved') }}</option>
                                        <option value="rejected" {{ old('status', $testimonial->status) == 'rejected' ? 'selected' : '' }}>{{ __('app.rejected') }}</option>
                                    </select>
                                    @error('status')
                                        <span class="text-danger small">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea name="text" id="text" rows="5" placeholder="{{ __('app.text') }}"
                                        class="form-control">{{ old('text', $testimonial->text) }}</textarea>
                                    @error('text')
                                        <span class="text-danger small">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="rate" id="rate" class="form-control form-select px-2">
                                        @for($i = 0; $i <= 5; $i++)
                                            <option value="{{ $i }}" {{ (string)old('rate', $testimonial->rate ?? 0) === (string)$i ? 'selected' : '' }}>
                                                {{ $i }}
                                            </option>
                                        @endfor
                                    </select>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="class_id" id="class_id" class="form-control form-select px-2">
                                        ...
                                        @foreach($classRooms as $classRoom)
                                            <option value="{{ $classRoom->id }}" {{ old('class_id', $testimonial->class_id) == $classRoom->id ? 'selected' : '' }}>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="file"
                                           id="image"
                                           name="image"
                                           data-default-file="{{ $testimonial->image }}"
...
<script>
    $(document).ready(function() {
        ...
        var selectedCourseId = {{ $testimonial->course_id ? $testimonial->course_id : 'null' }};
        var oldCourseId = {{ old('course_id', $testimonial->course_id) ? old('course_id', $testimonial->course_id) : 'null' }};
        var currentCourseId = oldCourseId || selectedCourseId;
                    ...
                    success: function(response) {
                        ...
                        if (response.courses && response.courses.length > 0) {
                            $.each(response.courses, function(index, course) {
                                var selected = (currentCourseId && course.id == currentCourseId) ? 'selected' : '';
